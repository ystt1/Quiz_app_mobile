...
class _DragAndDropState extends State<DragAndDrop> {
  ...
  void updateAnswer(String? answer, int dropIndex) {
    setState(() {
      ...
      for (int i = 0; i < droppedAnswers.length; i++) {
        if (droppedAnswers[i] == answer) {
          draggedFromIndex = i;
          break;
        }
      }
      ...
      if (draggedFromIndex != null) {
        droppedAnswers[draggedFromIndex] = swappedAnswer;
      }
      ...
      var updatedUserAnswers = widget.result.userAnswers.map((list) => List<String>.from(list)).toList();
      updatedUserAnswers[widget.index] = droppedAnswers.whereType<String>().toList();
